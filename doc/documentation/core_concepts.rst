...
- :class:`grakelx.datasets`
- :class:`grakelx.kernels`

The :class:`grakelx.datasets` subpackage contains functions for automatically downloading graph datasets. On the other hand, the :class:`grakelx.kernels` subpackage contains the :class:`grakelx.Kernel` class, and the implementations of all graph kernels and frameworks. Note that all graph kernels and frameworks such as the :class:`grakelx.ShortestPath` kernel or the :class:`grakelx.WeisfeilerLehman` framework inherit from the :class:`grakelx.Kernel` class.

GraKeL also contains the following two classes:

- :class:`grakelx.Graph`
- :class:`grakelx.GraphKernel`

The :class:`grakelx.Graph` class is used to represent graphs and also provides functions for manipulating them. The :class:`grakelx.GraphKernel` class is a generic wrapper. This class provides a uniform interface for all the implemented graph kernels and frameworks.

We next present some core components of *GraKeL*.

What is the :class:`grakelx.GraphKernel` Class?
----------------------------------------------
The :class:`grakelx.GraphKernel` class is a *generic wrapper class*. This class provides a uniform interface for all the implemented graph kernels and frameworks. A graph kernel can be described by an instance of this class, and it holds the attributes listed below:
...
* :code:`kernel` : Specifies the graph kernel to be computed. It can be either a :code:`base_graph_kernel` or a list that contains one or more :code:`framework` along with exactly one :code:`base_graph_kernel`. The :code:`base_graph_kernel` needs to be the last element in the list.
    ...
    .. code-block:: python
        ...
        >>> from grakelx import GraphKernel
...
We will next focus on the :class:`grakelx.Kernel` class. Instances of this class are wrapped in an instance of the :class:`grakelx.GraphKernel` class that was presented above.

The :class:`grakelx.Kernel` class
--------------------------------
...
The Figure below illustrates how the :class:`grakelx.Kernel` class is organized.
...
A kernel initialized as an instance of the :class:`grakelx.Kernel` class is equivalent to an instance of the :class:`grakelx.GraphKernel` generic wrapper corresponding to the same kernel if the attributes of the two kernels are identical to each other. To illustrate this, we will employ a deterministic graph kernel (the Wesfeiler-Lehman subtree kernel) and we will investigate if the kernel values produced by the two instances of the kernel are equal to each other.

We first initialize the instance of the :class:`grakelx.Kernel` class. This corresponds to the Weisfeiler-Lehman framework on top of the vertex histogram kernel.

.. code-block:: python

    >>> from grakelx import WeisfeilerLehman, VertexHistogram
    >>> gk_1 = WeisfeilerLehman(n_iter=5, base_graph_kernel=VertexHistogram)

We have set the :code:`base_graph_kernel` attribute equal to the :class:`grakelx.kernels.VertexHistogram` class. Note that the :code:`base_graph_kernel` attribute can also be set equal to a tuple consisting of a :class:`grakelx.kernel` class and a dictionary containing the attributes of the corresponding kernel and their values. Above, we have set the attributes of the vertex histogram kernel to their default values. Therefore, the above code is equivalent to the following.
...
We will now test if the kernel matrix produced by the instance of the :class:`grakelx.GraphKernel` class is equal to the one produced by the instance of the :class:`grakelx.Kernel` class.
...
The methods of the graph kernels take lists of :class:`grakelx.Graph` objects as input, extract the necessary features and may return some matrices. It should be mentioned that the :class:`grakelx.Kernel` objects are not allowed to modify the graphs that they take as input.
