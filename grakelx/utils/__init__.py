    ...
    def initialize(self):
        ...
        if not self._initialized["K"]:
            if self.K is None:
                ...
                if not flag:
                    raise ValueError("The provided K cannot be converted to a two dimensional np.array.")
            self.K_ = M
...
def graph_from_pandas(edge_df, node_df=None, directed=False, as_Graph=False):
    ...
    if (
        ...
        and (type(edge_df[2]) is tuple and len(edge_df[2]) == 2 and all(c in edge_df[0].columns for c in edge_df[2]))
...
def graph_from_csv(edge_files, node_files=None, index_type=str, directed=False, sep=",", as_Graph=False):
    ...
    if type(node_files) is None:
        nfs = None
    elif type(node_files) is tuple and len(node_files) == 2 and type(node_files[1]) in [bool, None]:
        nfs = list()
