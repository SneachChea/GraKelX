...
def generate_dataset(
    ...
    if (
        type(r_vertices) is not tuple
        or len(r_vertices) != 2
        or r_vertices[0] > r_vertices[1]
        or r_vertices[0] < 0
    ...
    if (
        ...
        or len(r_connectivity) != 2
        or r_connectivity[0] > r_connectivity[1]
        or r_connectivity[0] < 0
        or r_connectivity[1] > 1
        or not isinstance(r_connectivity[0], Real)
        or not isinstance(r_connectivity[1], Real)
    ):
        raise TypeError(
            "r_connectivity must be a Real tuple containing the minimum "
            "and the maximum percent of allowed connectivity in the range of [0, 1]"
        )

    if (
        type(r_weight_edges) is not tuple
        or len(r_weight_edges) != 2
        or r_weight_edges[0] > r_weight_edges[1]
        or r_weight_edges[0] <= 0
    ...
    for i in range(n_graphs):
        ...
        nedges = randint(low=int(max_ne * r_connectivity[0]), high=int(max_ne * r_connectivity[1]), size=(1,))
        if nedges == max_ne:
            ...
            if nedges > max_ne / 2:
                ...
                for i in range(size):
                    ...
                    for j in range(i + 1, size):
                        if c in grey_idxs:
                            A[i, j] = rand(low=r_weight_edges[0], high=r_weight_edges[1], size=(1,))[0].astype(float)
