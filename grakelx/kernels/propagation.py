        ...
        if not isinstance(X, Iterable):
            ...
            if self._method_calling == 1:
                ...
                if self._dim != P.shape[1]:
                    raise ValueError("transform attribute vectors shouldhave the same dimension as in fit")
                    ...
                    hd = dict(
                        chain(
                            ...
                            iter(
                                (j, i)
                                for i, j in enumerate(
                                    filterfalse(lambda x: x in self._hd[t], {tuple(l) for l in hashes}),
                                    len(self._hd[t]),
                                )
                            ),
                        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Measuring classification accuracy on multiscale_laplacian")
