...
from grakelx.kernels.shortest_path import ShortestPath
...
class CoreFramework(Kernel):
    ...
    Parameters
    ----------
    base_graph_kernel : `grakelx.kernels.kernel` or tuple, default=None
        ...
        if not self._initialized["base_graph_kernel"]:
            ...
            if base_graph_kernel is None:
                ...
                if not (type(base_graph_kernel) is type and issubclass(base_graph_kernel, Kernel)):
                    raise TypeError("The first argument must be a valid grakelx.kernel.kernel Object")
