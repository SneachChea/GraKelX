...
from grakelx.datasets import fetch_dataset, get_dataset_info
...
if __name__ == "__main__":
    ...
    parser.add_argument("--time", help="time the kernel computation (has ef" + "fect only on verbose)", action="store_true")
    parser.add_argument(
        "--problematic", help="allow execution of problemati" + "c test cases in development", action="store_true"
    )
...
if verbose:
    ...
    def test_random_walk():
        ...
        gk = GraphKernel(kernel={"name": "random_walk"}, verbose=verbose, normalize=normalize)
        if verbose:
            print_kernel_decorator("Random Walk", gk, dataset_tr, dataset_te)

    def test_shortest_path():
        """Test the Shortest Path kernel [decorator]."""
        gk = GraphKernel(kernel={"name": "shortest_path"}, verbose=verbose, normalize=normalize)
        if verbose:
    ...
    def test_graphlet_sampling():
        ...
        gk = GraphKernel(
            kernel={"name": "graphlet_sampling", "sampling": {"n_samples": 200}}, verbose=verbose, normalize=normalize
        )
    ...
    def test_weisfeiler_lehman():
        ...
        gk = GraphKernel(
            kernel=[{"name": "weisfeiler_lehman"}, {"name": "vertex_histogram"}], verbose=verbose, normalize=normalize
        )
        if verbose:
    ...
    def test_neighborhood_hash():
        ...
        gk = GraphKernel(kernel={"name": "neighborhood_hash"}, verbose=verbose, normalize=normalize)
        if verbose:
    ...
    def test_subgraph_matching():
        ...
        gk = GraphKernel(kernel={"name": "subgraph_matching"}, verbose=verbose, normalize=normalize)
        if verbose:
    ...
    def test_lovasz_theta():
        ...
        try:
            gk = GraphKernel(kernel={"name": "lovasz_theta"}, verbose=verbose, normalize=normalize)
        except ImportError:
            return
    ...
    def test_svm_theta():
        ...
        gk = GraphKernel(kernel={"name": "svm_theta"}, verbose=verbose, normalize=normalize)
    ...
    def test_propagation():
        ...
        gk = GraphKernel(kernel={"name": "propagation"}, verbose=verbose, normalize=normalize)
    ...
    def test_hadamard_code():
        ...
        gk = GraphKernel(kernel=[{"name": "hadamard_code"}, {"name": "subtree_wl"}], verbose=verbose, normalize=normalize)

        if verbose:
            print_kernel_decorator("Hadamard-Code/Subtree-WL [Simple]", gk, dataset_tr, dataset_te)
    ...
    def test_edge_histogram():
        ...
        if verbose:
            print_kernel_decorator("Edge Histogram", gk, dataset_tr, dataset_te)
    ...
    def test_vertex_histogram():
        ...
        if verbose:
            print_kernel_decorator("Vertex Histogram", gk, dataset_tr, dataset_te)
    ...
    def test_multiscale_laplacian():
        ...
        if verbose:
            print_kernel_decorator("Multiscale Laplacian", gk, dataset_attr_tr, dataset_attr_te)

    def test_graph_hopper():
        """Test the Graph Hopper kernel [decorator]."""
        gk = GraphKernel(kernel={"name": "graph_hopper"}, verbose=verbose, normalize=normalize)

        if verbose:
            print_kernel_decorator("Graph Hopper", gk, dataset_attr_tr, dataset_attr_te)
    ...
    def test_core_framework():
        ...
        if verbose:
            print_kernel_decorator("Core Framework", gk, dataset_tr, dataset_te)
